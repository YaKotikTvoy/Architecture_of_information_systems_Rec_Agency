...
if(isset($_POST["name"]) && isset($_POST["surname"]) && isset($_POST["middlename"])  && isset($_POST["phone"])  && isset($_POST["email"])){
	try{
		...
		$sql = "INSERT INTO Employee(name, surname, middlename, phone, email) 
		...
		$stmt -> execute();
		$idsql = "Select id_Employee from Employee where name = :username and surname = :usersurname and middlename = :usermiddlename and phone = :userphone and email = :useremail";
		/*$idsql = "Select * from Employee";*/
		...
		if($stmt -> rowCount() > 0){
			foreach($stmt as $row){
				echo "Пользователь успешно зарегистрирован.<br>Его id_Employee: <h1>" . $row["id_Employee"] . "</h1>";
			}
		}
		$conn = null;
	...
	header("Location: Create_password_form_Employee.php?invalid_password=true");
}
